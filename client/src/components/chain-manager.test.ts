...
describe('chain manager', () => {
  ...
  it('renames a chain through the dedicated endpoint, with a confirm (#1179)', () => {
    // A chain's name is the address clients route with (auto:<name>), so the
    // rename must go through /rename — where the creation rules are enforced
    // and protected chains get a real 403 — and must warn about in-flight
    // auto:<old-name> callers before committing.
